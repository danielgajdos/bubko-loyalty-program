...
CREATE DATABASE IF NOT EXISTS kebab_loyalty;
USE kebab_loyalty;
...
CREATE INDEX idx_user_stamps ON product_stamps(user_id, product_id);
CREATE INDEX idx_visits_user ON visits(user_id, visit_date);
CREATE INDEX idx_visit_items ON visit_items(visit_id, product_id);
